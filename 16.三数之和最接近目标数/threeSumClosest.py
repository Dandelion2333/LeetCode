...
class Solution
    def threeSumClosest_me(self, nums, target)
        # 入口参数检查
        if(len(nums)  3)
            return 0
        ...
        while value  len(nums)-1
            ...
            # 内层循环，控制left与right的位置
            while left  right
                tmp = nums[value] + nums[left] + nums[right]
                if tmp = target
                    right = right - 1
                else
                    left = left + 1
                
                # 判断当前三个数值之和是否离target最近
                Solution().is_closest(closest, tmp, target)
    ...
    # 获取离的最近的值
    def is_closest(self, closest, tmp, target)
        gap = target - tmp
        if abs(gap) = closest[0]
            closest[0] = abs(gap)
            closest[1] = tmp
            print(gap, abs(gap), closest, closest)


# 双指针移动的时候会去重
# 当三数之和等于target的时候，直接就return。
class Solution_study
    def threeSumClosest_study(self, nums, target)
        ...
        # 获取最接近target的三数之和
        def is_closest(current)
            nonlocal best
            if abs(target - current)  abs(target - best)
                best = current
                # print(best, best, abs(target-current))
        ...
        for start in range(num)
            ...
            while left  right
                ...
                if sum_val  target
                    if num  50 and (right-left)20
                        # 采用二分法，看能不能缩小时间复杂度
                        right_mid = round((right + left)2)
                        sum_val = nums[start] + nums[left] + nums[right_mid]
                        # print(sum_val, right_mid)
                        if sum_val  target and right_mid = right and right_mid  left
                            right = right_mid
                    ...
                    if num  50 and (right-left)40
                        # 采用二分法，看能不能缩小时间复杂度
                        left_mid = round((right+left)2)
                        sum_val = nums[start] + nums[left] + nums[left_mid]
                        if sum_val  target and left_mid  left and left_mid  right
                            left = left_mid
